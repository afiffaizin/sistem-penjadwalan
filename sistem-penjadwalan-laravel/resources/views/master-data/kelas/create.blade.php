...
<div class="container mx-auto max-w-2xl">
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-8">
        <h2 class="text-2xl font-bold text-gray-800 mb-6 border-b border-gray-100 pb-4">Tambah Kelas Baru</h2>
        ...
        <form action="{{ route('kelas.store') }}" method="POST">
            ...
            <div class="mb-5">
                ...
                <input type="text" name="nama" value="{{ old('nama') }}" class="w-full border border-gray-300 rounded-lg shadow-sm focus:border-amber-500 focus:ring-amber-500 px-4 py-2.5 outline-none transition @error('nama') border-red-500 @enderror" placeholder="Contoh: RKS-3A">
            ...
            <div class="mb-8">
                <label class="block text-sm font-bold text-gray-700 mb-2">Tahun Ajar Aktif</label>
                <select name="tahun_ajar_id" class="w-full border border-gray-300 rounded-lg shadow-sm focus:border-amber-500 focus:ring-amber-500 px-4 py-2.5 outline-none transition bg-white @error('tahun_ajar_id') border-red-500 @enderror">
                    ...
                    @foreach($tahunAjars as $ta)
                        <option value="{{ $ta->id }}" {{ old('tahun_ajar_id') == $ta->id ? 'selected' : '' }}>
                            {{ $ta->tahun }} - {{ $ta->semester }} @if($ta->is_active) (AKTIF) @endif
                        </option>
                    @endforeach
                </select>
            ...
            <div class="flex justify-end gap-3 pt-4 border-t border-gray-100">
                ...
                <button type="submit" class="px-5 py-2.5 bg-amber-500 text-white rounded-lg hover:bg-amber-600 font-bold shadow-md shadow-amber-200 transition">Simpan Kelas</button>
            </div>
        </form>
    </div>
</div>
