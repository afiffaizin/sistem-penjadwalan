...
<div class="container mx-auto max-w-7xl">
    <div class="bg-white rounded-xl shadow-sm p-6 mb-6 border-l-8 border-amber-600">
        ...
        <p class="text-gray-500 text-sm mb-6">Jadwal di bawah ini dikelompokkan berdasarkan rombongan kelas pada program studi Anda.</p>
        ...
        <form action="{{ route('kaprodi.jadwal') }}" method="GET">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mt-4 pt-4 border-t border-gray-100">
                ...
                <div class="flex gap-3">
                    <a href="{{ route('jadwal.export.excel', array_merge(request()->all(), ['prodi_id' => auth()->user()->prodi_id])) }}" 
                    ...
                    <a href="{{ route('jadwal.export.pdf', array_merge(request()->all(), ['prodi_id' => auth()->user()->prodi_id])) }}" 
                ...
                <div class="flex gap-3">
                    <a href="{{ route('kaprodi.jadwal') }}" class="px-5 py-2.5 bg-gray-100 text-gray-600 rounded-lg text-sm font-bold hover:bg-gray-200 transition flex items-center">
                        Reset
                    </a>
                    <button type="submit" class="px-6 py-2.5 bg-amber-600 text-white rounded-lg text-sm font-bold hover:bg-amber-700 transition shadow-sm flex items-center">
                        ...
                        <tbody class="text-xs">
                            @for($s = 1; $s <= $totalSesi; $s++)
                                <tr>
                                    ...
                                    @foreach($hariKerja as $hari)
                                        @php 
                                            $jadwalSelIni = collect($matrixJadwal[$s][$hari])->where('kelas', $k->nama); 
                                        @endphp
                                        ...
                                        <td class="border border-gray-200 p-1.5 align-top bg-white">
                                            <div class="flex flex-col gap-2 h-full">
                                                @forelse($jadwalSelIni as $jadwal)
                                                    @php
                                                        $hexColor = $colorMap[$jadwal['warna'] ?? 'bg-blue-50'] ?? '#eff6ff';
                                                    @endphp
                                                    <div style="background-color: {{ $hexColor }};" class="border border-black/10 rounded p-1.5 text-left shadow-sm">
                                                        ...
                                                        <div class="flex flex-col gap-1 mt-1 border-t border-black/10 pt-1">
                                                            <span class="text-[10px] font-bold text-gray-800 truncate"><i class="fa-solid fa-user-tie opacity-50 mr-0.5"></i> {{ explode(',', $jadwal['dosen'])[0] }}</span>
                                                            <div class="flex justify-between items-center text-[9px] font-semibold text-gray-700">
                                                                ...
                                                                <span class="px-1 py-0.5 rounded text-white font-bold {{ strtolower($jadwal['jenis']) == 'praktikum' ? 'bg-indigo-600' : 'bg-amber-600' }}">{{ $jadwal['jenis'] }}</span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                @empty
