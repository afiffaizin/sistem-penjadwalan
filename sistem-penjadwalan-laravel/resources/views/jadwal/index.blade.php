...
@section('content')
    @php
        $jadwalDicari =
            request()->has('tahun_ajar_id') || request()->anyFilled(['dosen_id', 'kelas_id', 'ruang_id', 'prodi_id']);
    ...
    <div class="container mx-auto max-w-7xl" x-data="{ editMode: false }">
        ...
        @if (session('success'))
            <div
                class="bg-green-50 border border-green-200 text-green-700 px-5 py-4 rounded-xl mb-6 text-sm font-bold shadow-sm flex items-center gap-3">
        ...
        @if (session('error'))
            <div
                class="bg-red-50 border border-red-200 text-red-700 px-5 py-4 rounded-xl mb-6 text-sm font-bold shadow-sm flex items-start gap-3">
        ...
        <div class="bg-white rounded-xl shadow-sm p-6 mb-6 border-l-8 border-amber-500">
            ...
            <form id="formFilterJadwal" action="{{ url()->current() }}" method="GET">
                ...
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                    <div>
                        <h1 class="text-2xl font-extrabold text-gray-800 tracking-tight mb-2">Pencarian Jadwal Perkuliahan
                        </h1>
                        <p class="text-gray-500 text-sm">Gunakan satu atau kombinasi filter di bawah untuk memunculkan
                            jadwal perkuliahan.</p>
                    </div>

                    <div
                        class="flex flex-col sm:flex-row items-start sm:items-end gap-3 w-full md:w-auto self-end md:self-center justify-end">
                        ...
                        @if ($jadwalDicari)
                            @if (auth()->check() && auth()->user()->role === 'sekretaris')
                                <button type="button" @click="editMode = !editMode"
                                    :class="editMode ? 'bg-rose-600 hover:bg-rose-700 shadow-rose-100' :
                ...
                <div
                    ...
                    <div class="flex w-full sm:w-auto justify-end gap-3">
                        ...
                        <button type="submit"
                            class="px-6 py-2.5 bg-amber-500 text-white rounded-lg text-sm font-bold hover:bg-amber-600 transition shadow-sm">
                                                            ...
                                                            <div class="flex flex-col gap-2 h-full">
                                                                @forelse($jadwalSelIni as $jadwal)
                                                                    ...
                                                                    <div style="background-color: {{ $hexColor }};"
                                                                        class="relative group border border-black/10 rounded p-1.5 text-left transition hover:shadow-md shadow-sm">

                                                                        <button x-show="editMode" style="display: none;"
                                                                            onclick="openEditModal({{ json_encode($jadwalId) }}, {{ json_encode($jadwal['mata_kuliah']) }}, {{ json_encode($hariJadwal) }}, {{ json_encode($sesiMulai) }}, {{ json_encode($durasi) }})"
                            ...
                            <table class="w-full text-sm border-collapse border border-gray-200 text-center">
                                ...
                                <tbody>
                                    @for ($s = 1; $s <= $totalSesi; $s++)
                                        <tr>
                                            <td
                                                class="border border-gray-200 py-3 font-extrabold bg-gray-50 text-gray-700">
                                                {{ $s }}</td>
                                            @foreach ($hariKerja as $hari)
                                                <td class="border border-gray-200 p-2 align-top bg-white">
                                                    <div class="flex flex-col gap-2 h-full">
                                                        @forelse($matrixJadwal[$s][$hari] as $jadwal)
                                                            ...
                                                            <div style="background-color: {{ $hexColor }};"
                                                                class="relative group border border-black/10 rounded p-2 text-left transition hover:shadow-md shadow-sm">
    ...
    <div id="editScheduleModal" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center"
        style="display: none;">
        <div class="bg-white rounded-xl shadow-lg max-w-md w-full overflow-hidden border border-gray-100 relative">
            ...
            <form action="{{ route('jadwal.proses-ubah') }}" method="POST" class="p-6 space-y-4">
                ...
                <div
                    class="bg-indigo-50 border border-indigo-100 p-3 rounded-lg text-xs text-indigo-900 font-semibold mb-2">
                    <i class="fa-solid fa-circle-info mr-1"></i> Durasi mata kuliah ini adalah <span id="modalDurasiText"
                        class="text-indigo-600 font-bold">0</span> sesi. Seluruh blok sesi ini akan digeser secara
                    bersamaan.
                </div>

                <div>
                    <label class="block text-[11px] font-extrabold text-gray-500 uppercase tracking-wider mb-1.5">Pindahkan
                        ke Hari</label>
                    <select name="hari_baru" id="modalHariSelect"
                        class="w-full border border-gray-300 rounded-lg p-2.5 text-sm bg-white outline-none focus:ring-2 focus:ring-indigo-500/50"
                        required>
                ...
                <div>
                    <label class="block text-[11px] font-extrabold text-gray-500 uppercase tracking-wider mb-1.5">Mulai
                        dari Sesi Ke-</label>
                    <select name="sesi_mulai_baru" id="modalSesiSelect"
                        class="w-full border border-gray-300 rounded-lg p-2.5 text-sm bg-white outline-none focus:ring-2 focus:ring-indigo-500/50"
                        required>
                ...
                <div class="pt-2 border-t border-gray-100">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="mode_tukar" value="1"
                            class="rounded text-indigo-600 focus:ring-indigo-500">
                        <span class="text-xs font-bold text-gray-700">Tukar Posisi (Gunakan opsi ini jika slot tujuan
                            menabrak kelas lain)</span>
                    </label>
                </div>

                <div class="flex justify-end gap-3 pt-4 border-t border-gray-100">
                    <button type="button" onclick="closeEditModal()"
                        class="px-5 py-2.5 bg-gray-100 text-gray-600 rounded-lg text-sm font-bold hover:bg-gray-200 transition">Batal</button>
                    <button type="submit"
                        class="px-6 py-2.5 bg-indigo-600 text-white rounded-lg text-sm font-bold hover:bg-indigo-700 transition shadow-sm">
